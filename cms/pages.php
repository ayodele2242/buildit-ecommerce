...
if(isset($_GET['delete'])){
    ...
    $myqli = mysqli_query($mysqli,"delete from pages where id = '$id'");
    if($mysqli){
        setFlash('success',"Page info deleted successfully.");
    }else{
        setFlash('error',"Page Info cannot be deleted.");
    }
}
                          ...
                          foreach ($page_info as $key => $pg_info) {
                            ...
                            <tr>
                              ...
                              <td><?php echo ($pg_info->status == 1)?'Active':'Inactive'; ?></td>
                                <td>
                                 <button class="btn-floating waves-effect waves-light gradient-45deg-amber-amber small edit-page" value='<?php echo json_encode($pg_info); ?>'><i class="fa fa-pencil"></i></button>
                                  <?php 
                                  $url="process/pages?id=".$pg_info->id."&act=".substr(md5($_SESSION['session_id'].'del_page-'.$pg_info->id), 5, 15)
                                  ?>
                                  <a href="<?php echo $url; ?>" class="btn-floating waves-effect waves-light gradient-45deg-red-pink small" onclick= "return confirm('Are you sure you want to delete this page?');"><i class="fa fa-trash"></i></a>
                                </td>
...
<script type="text/javascript">
  ...
  $('.edit-page').click(function(e){
    ...
    if (current_tag != "") {
      ...
      $(tinymce.get('description').getBody()).html(page_info.description);
      //tinymce.activeEditor.selection.setContent(page_info.description);
      //tinymce.get('description').setContent(page_info.description);
      //tinymce.get("description").getBody().innerHTML = page_info.description;
