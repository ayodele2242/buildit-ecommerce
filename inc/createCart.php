...
$sessionId = $_POST['mysession'];
}else{
$sessionId = session_id();
}
//check if product exist in the database, if it does, update it

$get = mysqli_query($mysqli,"select * from cart where product_id = '$product_id' AND color = '$color' AND size = '$product_size' AND sessionId = '$sessionId'");
$count = mysqli_num_rows($get);
if($count > 0){
	$update = mysqli_query($mysqli,"UPDATE cart SET quantity = quantity+$qty WHERE product_id = '$product_id' AND color = '$color' AND size = '$product_size' AND sessionId = '$sessionId' ");
...
values('$product_id','$title','$cat','$color','$product_size','$amount','$vendor','$img','$sessionId','$qty') ");
